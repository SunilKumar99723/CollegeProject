...
class nodetab extends JPanel implements ActionListener,ItemListener
{
	...
	nodetab(nodeD parent)
	{
		super();
		
		myparent=parent;
		new nodeDServer(myparent);
	...
	Vector sendroutereq(String destination)
	{
		...
		try
		{
			...
			if (destination.equals("nodeA"))
			{
				String finalmsg="ROUTEREQUEST&nodeD&"+destination;
				System.out.println(finalmsg);
				
				Socket s=new Socket(nodeA,1500);
			...
			if (destination.equals("nodeC"))
			{
				...
				Socket s=new Socket(nodeC,3500);
				DataOutputStream dout=new DataOutputStream(s.getOutputStream());
				dout.writeUTF(finalmsg);
			...
			if (destination.equals("nodeB"))
			{
				...
				Socket s1=new Socket(nodeA,1500);
				...
				Socket s2=new Socket(nodeC,3500);
	...
	void sendmsg(String route1,String msg,String destination)
	{
		...
		//	String routearray[]=route.split("^");
			...
			if (v.get(1).toString().equals("nodeA"))
			{
				Socket s=new Socket(nodeA,1500);
				DataOutputStream dout=new DataOutputStream(s.getOutputStream());
				dout.writeUTF(finalmsg);
